...
BEGIN
	...
	SELECT dbo.Person_Main.Employee_Cd AS Id, 
		  ...
		  -- OriginalHire_Dt, 
		   (SELECT COUNT(1) FROM dbo.Assignment WHERE [Status] = 0 AND DueDate <= @Today AND Person_Main.Employee_Cd = Assignment.Employee_Cd) AS 'Open',
		   (SELECT COUNT(1) FROM dbo.Assignment WHERE [Status] = 0 AND DueDate > @Today AND Person_Main.Employee_Cd = Assignment.Employee_Cd) AS 'Overdue',
		   (SELECT COUNT(1) FROM dbo.Assignment WHERE [Status] = 1 AND Person_Main.Employee_Cd = Assignment.Employee_Cd) AS 'Closed',
